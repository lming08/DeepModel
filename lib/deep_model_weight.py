...
class DeepModel(object):
    ...
    def model_fn(self, features, labels, mode, params):
        ...
        with tf.name_scope("input"):
            input_layer_weight = tf.feature_column.input_layer(features, feature_columns_weight)
            ...
            sample_weight = tf.reshape(input_layer_weight, [-1, ])
        ...
        for i, dim in enumerate(hidden_units):
            deep_output = tf.layers.dense(inputs=deep_output, units=dim, use_bias=True, kernel_initializer=tf.random_uniform_initializer(-0.001, 0.001))
    ...
    def train(self):
        ...
        self.model.train(input_fn=self.train_input_config.input_weight_fn, steps=None)

    def evaluate(self):
        #hooks = [tf_debug.LocalCLIDebugHook()]
        result = self.model.evaluate(input_fn=self.test_input_config.input_weight_fn, steps=None)

    def train_and_evaluate(self):
        tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.INFO)
        train_spec = tf.estimator.TrainSpec(input_fn=self.train_input_config.input_weight_fn)
        eval_spec = tf.estimator.EvalSpec(input_fn=self.test_input_config.input_weight_fn, throttle_secs=0)
        tf.estimator.train_and_evaluate(self.model, train_spec, eval_spec)

    def predict(self):
        result = self.model.predict(input_fn=self.test_input_config.input_weight_fn)
        return result

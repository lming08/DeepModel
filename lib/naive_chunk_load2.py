...
class NaiveChunkLoad2(object):
    ...
    def train(self, train_file, valid_file, batch_size, block_num, epochs, verb, model_dir):
        ...
        for epoch in range(0, epochs):
            ...
            for sub_data in pd.read_csv(train_file, sep='\t', names=columns, usecols=usecols, chunksize=batch_size*10000):
                batchs = self._get_batch(sub_data, batch_size, is_sample=False, is_norm=self.is_norm)
                logging.info('epoch [{}] finish get_batch'.format(epoch))
                for df_batch in batchs:
                    ...
                    if verb > 0 and step % verb == 0:
                        logging.info('epoch [{}]\t{} steps\tloss={}'.format(epoch, step, loss_sum / verb))
                        loss_sum = 0
            is_print_train_res = False
            if is_print_train_res:
                train_labels, train_preds = self._predict(train_file, batch_size*300, 50, None, 'train', 0, names=columns, usecols=usecols)
            ...
            valid_labels, valid_preds = self._predict(valid_file, batch_size*300, 50, None, 'valid', 0, names=columns, usecols=usecols)
    ...
    def _predict(self, data_file, batch_size, block_num, metrics=None, pred_type='test', verb=100, names=None, usecols=None):
        if pred_type not in ['test', 'train', 'valid']:
            raise ValueError("pred_type not in ['test', 'train', 'valid']")
        if pred_type == 'test':
            self.sess.run(tf.global_variables_initializer())
        ...
        labels = list()
        num = 0
        for sub_data in pd.read_csv(data_file, sep='\t', names=names, usecols=usecols, chunksize=batch_size*block_num):
                batchs = self._get_batch(sub_data, batch_size, is_sample=False, is_norm=self.is_norm)
                for df_batch in batchs:
                    tmp_preds = self._predict_batch(df_batch, self.sess)
                    preds += tmp_preds

...
class NaiveChunkLoad3(object):
    ...
    def dense_auto_project_layer(self, inputs, split_num, split_axis, unit, initializer, is_batch_normal, name='auto_project'):
        ...
        for sp in splits:
            if is_batch_normal:
                sp = tf.layers.batch_normalization(sp, training=self.training)
    ...
    def _build_deep(self, input, layer_dim_lst, activation, initializer, regularizer, dropout_prob):
        ...
        for i, dim in enumerate(layer_dim_lst):
            if self.batch_norm:
                deep_output = tf.layers.dense(inputs=deep_output, units=dim, kernel_initializer=initializer, 
                    kernel_regularizer=regularizer, name='deep_input_' + str(i + 1))
                deep_output = tf.layers.batch_normalization(deep_output, training=self.training)
    ...
    def train(self, train_file, valid_file, batch_size, block_num, epochs, verb, model_dir):
        ...
        train_dataset = self._cvt_tfrecord_load_data(train_file, columns, usecols, num_epochs=epochs)
        valid_dataset = self._cvt_tfrecord_load_data(valid_file, columns, usecols, num_epochs=1)

        self.sess.run(tf.global_variables_initializer())
        self.sess.run(tf.local_variables_initializer())

        train_dataset = train_dataset.repeat(1).batch(batch_size).prefetch(buffer_size=1024)
        ...
        try:
            loss_sum = 0
            while True:
                batch_arr = self.sess.run(next_element)
        ...
        valid_dataset = valid_dataset.repeat(1).batch(batch_size).prefetch(buffer_size=1024)
        ...
        try:
            while True:
                batch_arr = self.sess.run(next_element)
                tmp_preds = _predict_batch(arr_data, self.sess)
                valid_preds += tmp_preds
                valid_labels += batch_arr[:,0].tolist()
        except tf.errors.OutOfRangeError:
    ...
    def _train_batch(self, arr_train, sess):
        ...
        feed_dict[self.labels] = arr_train[:, 0:1]
    ...
    def _cvt_tfrecord_load_data(self, data_file, columns, usecols, num_epochs=2):
        def parse_line(line):
            fields = tf.decode_csv(line, record_defaults=[0.]*len(columns), field_delim="\t")
            instance = tf.cast(fields, tf.float64)
            instance = tf.reshape(instance, [len(columns), ])
            return instance

        '''
        filename_queue = data_file
        files = tf.data.Dataset.list_files(filename_queue)
        dataset = files.interleave(tf.data.TextLineDataset)
        dataset = dataset.map(map_func = parse_line)
        '''

        dataset = tf.data.TextLineDataset(data_file, num_parallel_reads=32).map(parse_line)

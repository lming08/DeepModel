...
class NaiveChunkLoad3(object):
    ...
    def train(self, train_file, valid_file, batch_size, block_num, epochs, verb, model_dir):
        ...
        try:
            while True:
                batch_arr = sess.run(next_element)
        ...
        valid_labels, valid_preds = self._predict_with_queue(batch_valid_instance, valid_batch_size, 0, None, 'valid', 200, columns=columns)
    ...
    def _cvt_tfrecord_load_data(self, data_file, columns, usecols, num_epochs=2):
        def parse_line(line):
            fields = tf.decode_csv(line, field_delim=','="\t")

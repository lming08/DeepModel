...
class DeepFMModel(object):
    ...
    def model_fn(self, features, labels, mode, params):
        ...
        feature_columns_emb= [tf.feature_column.embedding_column(tf.feature_column.categorical_column_with_identity(col_name, num_buckets=cate_fea_num+1, default_value=0), \
        ...
        # first-order
        first_order_feature_columns = [tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity(col_name, num_buckets=cate_fea_num+1, default_value=0)) \
        ...
        with tf.name_scope("input"):
            ...
            input_layer_emb = tf.feature_column.input_layer(features, feature_columns_emb)
            input_layer_fo = tf.feature_column.input_layer(features, first_order_feature_columns) #onehot
            print "---------- raw_input_layer_dense", input_layer_dense
            print "---------- raw_input_layer_emb", input_layer_emb
            print "---------- raw_input_layer_fo", input_layer_fo
        ...
        cate_feature_emb = tf.reshape(input_layer_emb, [-1, cate_feature_num, self._get_emb_dim()])
        print "----------", cate_feature_emb

        # second-order
        summed_cate_feature_emb_square = tf.square(tf.reduce_sum(cate_feature_emb, 1))
        squared_sum_cate_feature_emb = tf.reduce_sum(tf.square(cate_feature_emb), 1)
        second_order = 0.5 * tf.subtract(summed_cate_feature_emb_square, squared_sum_cate_feature_emb) #None*K

        first_order = tf.layers.dense(inputs=input_layer_fo, units=1, use_bias=True, kernel_initializer=tf.compat.v1.glorot_uniform_initializer())
        first_order = tf.nn.relu(first_order)
        #first_order = tf.layers.batch_normalization(first_order, momentum = 0.9, training=is_training)

        hidden_units = params['hidden_units']
        input_layer_dense = tf.layers.batch_normalization(input_layer_dense, momentum = 0.9, training=is_training)
        if len(params["mult_feature_names"]) > 0:
            deep_output = tf.concat([input_layer_dense, input_layer_emb, average_emb_mult], axis=1)
        else:
            deep_output = tf.concat([input_layer_dense, input_layer_emb], axis=1)
        ...
        deep_output = tf.concat([first_order, second_order, deep_output], axis=1)
    ...
    def train(self):
        ...
        self.model.train(input_fn=input_fn, steps=None, hooks=None)
    ...
    def evaluate(self):
        ...
        result = self.model.evaluate(input_fn=input_fn, steps=None)
